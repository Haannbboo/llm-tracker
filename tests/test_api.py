...
def test_build_usage_query_with_provider_filter(api_module):
    ...
    assert query == "SELECT * FROM usage WHERE provider = ? ORDER BY id DESC LIMIT ?"
    assert params == ("vectorengine", 10)
...
def test_build_usage_query_with_model_filter(api_module):
    ...
    assert query == "SELECT * FROM usage WHERE model = ? ORDER BY id DESC LIMIT ?"
    assert params == ("gpt-5.4-medium", 10)
