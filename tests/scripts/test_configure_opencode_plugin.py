...
SCRIPT = (
    Path(__file__).resolve().parents[2] / "scripts" / "configure-opencode-plugin.py"
)
...
def test_configure_opencode_plugin_registers_endpoint(tmp_path):
    home = tmp_path / "home"
    home.mkdir()
    ...
    assert plugin_entry == [
        str(project_root / "plugins" / "opencode" / "dist" / "index.js"),
        {"endpoint": "http://localhost:4102/v1/logs"},
    ]


def test_configure_opencode_plugin_updates_existing_entry(tmp_path):
    home = tmp_path / "home"
...
def test_configure_opencode_plugin_skips_without_npm(tmp_path):
    home = tmp_path / "home"
    home.mkdir()
