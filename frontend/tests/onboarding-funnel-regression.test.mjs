...
// These patterns only appear in App.tsx JSX, not in the test.
...
describe('onboarding 3-step funnel', () => {
  it('shows three numbered steps in the onboarding section', () => {
    assert.ok(appSrc.includes(step1T), 'Expected Step 1 t() call in onboarding');
    assert.ok(appSrc.includes(step2T), 'Expected Step 2 t() call in onboarding');
    assert.ok(appSrc.includes(step3T), 'Expected Step 3 t() call in onboarding');
  });

  it('Step 1 is bootstrap (not agent command rows)', () => {
    const area = sectionBetween(appSrc, step1T, step2T);
    assert.ok(
  ...
  it('Step 2 is run a test command (existing agent rows)', () => {
    const area = sectionBetween(appSrc, step2T, step3T);
    assert.ok(
  ...
  it('Step 3 waits for event automatically', () => {
    const area = sectionAfter(appSrc, step3T);
    assert.ok(
